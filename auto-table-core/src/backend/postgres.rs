...
mod tests {
    ...
    use super::{normalize_postgres_default, normalize_postgres_type};

    #[test]
    fn postgres_adds_a_plain_index() {
        assert_eq!(
            postgres_plan(
                vec![],
                vec![IndexChange::Add(IndexSchema {
    ...
    #[test]
    fn postgres_adds_and_drops_a_primary_key_constraint() {
        assert_eq!(
            postgres_plan(
                vec![],
                vec![IndexChange::Add(IndexSchema {
            ...
            vec!["ALTER TABLE \"users\" ADD PRIMARY KEY (\"id\")"]
        );
        assert_eq!(
            postgres_plan(
                vec![],
    ...
    #[test]
    fn postgres_drops_a_plain_index() {
        assert_eq!(
            postgres_plan(
                vec![],
                vec![IndexChange::Drop(IndexSchema {
            ...
            vec!["DROP INDEX \"email\""]
        );
    }

    #[test]
    fn postgres_sets_and_drops_a_default() {
        assert_eq!(
            postgres_plan(
                vec![ColumnChange::Alter {
                    name: "age".to_string(),
            ...
            vec!["ALTER TABLE \"users\" ALTER COLUMN \"age\" SET DEFAULT '0'"]
        );
        assert_eq!(
            postgres_plan(
                vec![ColumnChange::Alter {
    ...
    #[test]
    fn postgres_makes_a_column_nullable() {
        assert_eq!(
            postgres_plan(
                vec![ColumnChange::Alter {
                    name: "age".to_string(),
                    to: column("age", "int"),
                    aspects: vec![ColumnAspect::Nullable { from: false, to: true }],
                }],
                vec![],
            ),
    ...
    #[test]
    fn postgres_toggles_identity() {
        assert_eq!(
            postgres_plan(
                vec![ColumnChange::Alter {
                    name: "id".to_string(),
                    to: column("id", "bigint"),
                    aspects: vec![ColumnAspect::AutoIncrement { from: false, to: true }],
                }],
                vec![],
            ),
            vec!["ALTER TABLE \"users\" ALTER COLUMN \"id\" ADD GENERATED BY DEFAULT AS IDENTITY"]
        );
        assert_eq!(
            postgres_plan(
                vec![ColumnChange::Alter {
                    name: "id".to_string(),
                    to: column("id", "bigint"),
                    aspects: vec![ColumnAspect::AutoIncrement { from: true, to: false }],
                }],
                vec![],
            ),
            vec!["ALTER TABLE \"users\" ALTER COLUMN \"id\" DROP GENERATED BY DEFAULT AS IDENTITY"]
        );
    }

    #[test]
    fn postgres_keeps_the_given_aspect_order() {
        assert_eq!(
            postgres_plan(
                vec![ColumnChange::Alter {
                    name: "age".to_string(),
                    to: column("age", "int"),
                    aspects: vec![
                        ColumnAspect::Type { from: "int".to_string(), to: "bigint".to_string() },
                        ColumnAspect::Nullable { from: false, to: true },
                        ColumnAspect::Default { from: Some("0".to_string()), to: None },
                    ],
                }],
                vec![],
            ),
    ...
    #[test]
    fn normalize_postgres_type_maps_reported_spellings() {
        assert_eq!(normalize_postgres_type("character varying"), "varchar");
        assert_eq!(normalize_postgres_type("numeric"), "decimal");
        assert_eq!(normalize_postgres_type("boolean"), "bool");
        assert_eq!(normalize_postgres_type("timestamp"), "timestamp");
    }

    #[test]
    fn normalize_postgres_default_strips_the_type_annotation() {
        assert_eq!(
            normalize_postgres_default("'member'::character varying"),
            Some("member".to_string())
        );
        assert_eq!(normalize_postgres_default("'active'"), Some("active".to_string()));
        assert_eq!(normalize_postgres_default(""), None);
        assert_eq!(normalize_postgres_default("nextval('users_id_seq')"), None);
        assert_eq!(normalize_postgres_default("now()"), None);
    }
